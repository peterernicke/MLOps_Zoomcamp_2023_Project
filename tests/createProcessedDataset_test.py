...
def test_createDatasets():
    ...
    if not os.path.isfile(FULL_TRAIN_PATH):    
        orch.create_processed_dataset(category="A", train_path=FULL_TRAIN_PATH, val_path=FULL_VAL_PATH, test_path=FULL_TEST_PATH)
        orch.create_processed_dataset(category="H", train_path=TRAIN_PATH, val_path=VAL_PATH, test_path=TEST_PATH)
        orch.create_processed_dataset(category="C", train_path=PROBLEM_TRAIN_PATH, val_path=PROBLEM_VAL_PATH, test_path=PROBLEM_TEST_PATH)
